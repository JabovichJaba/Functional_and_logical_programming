...
%EX4
problem(windows) :-
    query('Does the system crash when you turn it on?').

problem(processor-lite) :-
    query('Do heavy programs work unstably?').

problem(processor-hard) :-
    query('Does the processor temperature exceed 70 degrees during short-term operation?').   
%Add-que
fault(term-proccessor) :-
    problem(processor-lite),
    problem(processor-hard).
%Singl-que
fault(windows_crash) :-
